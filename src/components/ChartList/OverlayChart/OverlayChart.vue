...
export default {
  ...
  methods: {
    init() {
      ...
      const series = this.displayList.map((lineData, index) => {
        const chartData = isEmpty(this.depthLimit)
          ? cloneDeep(lineData.chartData).slice(foundMinIndex, foundMaxIndex)
          : lineData.chartData;
      ...
      let option = {
        ...
        tooltip: {
          ...
          formatter: (e1, e2, e3, e4) => {
            return e1
              .map(
                item =>
                  `<div style="display: flex;justify-content: space-between;">${item.marker}${item.value}%</div>`
              )
              .join("");
          }
        },
